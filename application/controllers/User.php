...
class User extends CI_Controller
{
    ...
    public function pria(){
        $data['judul'] = "Fashion Pria";
        $data['user'] = $this->ModelUser->cekData(['email' => $this->session->userdata('email')])->row_array();
        $data['produk'] = $this->ModelBarang->dataPria()->result();
        $this->load->view('user/userheader', $data);
        $this->load->view('produk/pria', $data);
        $this->load->view('footer', $data);
    }
    public function wanita(){
        $data['judul'] = "Fashion Wanita";
        $data['user'] = $this->ModelUser->cekData(['email' => $this->session->userdata('email')])->row_array();
        $data['produk'] = $this->ModelBarang->dataWanita()->result();
        $this->load->view('user/userheader', $data);
        $this->load->view('produk/wanita', $data);
        $this->load->view('footer', $data);
    }
    public function anak(){
        $data['judul'] = "Fashion Anak";
        $data['user'] = $this->ModelUser->cekData(['email' => $this->session->userdata('email')])->row_array();
        $data['produk'] = $this->ModelBarang->dataAnak()->result();
        $this->load->view('user/userheader', $data);
        $this->load->view('produk/anak', $data);
        $this->load->view('footer', $data);
    }
